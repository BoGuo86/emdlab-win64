...
function emdlab_g2d_lib_tc10(g, ID, OD, Ns, Nc, wc, dc, bs0, hs0, tta, h0x, h0y, name1, name2, name3)
...
e5 = g.addSegmentByCoordinates(e4h.ptr.p1.x,e4h.ptr.p1.y,ID/2,0);
...
indexList = g.addSegmentByCoordinates(e4h.ptr.p1.x,0,e4h.ptr.p1.x+ h0x,0);
xtmp = e4h.ptr.p1.x + h0x;
for i = 1:Nc
    ...
    s5Index = g.addSegmentByCoordinates(xtmp+dc,0,xtmp+dc+h0x,0);
        ...
        s5Index = g.addSegmentByCoordinates(xtmp+dc,0,OD/2,0);
    end
...
e1tmp = g.addArcByCoordinates(0,0,OD/2,0,x1,y1,1);
indexList = [indexList,e1tmp,e1,e2,e3,e4];
