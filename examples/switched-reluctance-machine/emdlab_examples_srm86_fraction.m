%{
note: magneto-static analysis of a 8/6 SRM by exciting one stator phase 
for a specific rotor position -> fraction model
%}
...
f_mesh = @(r) interp1([gv_Dsh/2,gv_ISD/2,gv_OSD/2], [gv_wry/2,0.5,gv_wsy/2], r);
...
% construct full mesh
m.aux_cmxjcrj('Rotor', gv_Nr/2, gv_alpharp);
m.aux_cmxjcrj('Stator', gv_Ns/2, gv_alphasp);
m.aux_cmxjcrj('RotorAP', gv_Nr/2, gv_alpharp);
m.aux_cmxcr('sca', gv_Ns/2, gv_alphasp);
