% example information:
% transient analysis of a single-phase transformer
% primary coil is connected to the voltage source
% secondary coil is short circuit -> short circuit test
...
gv_Np = 250; % number of turns in primary coil
gv_Vp = 24; % peak voltage of primary coil
gv_freq = 50; % excitation frequency
gv_Ns = 500; % number of turns in secondary coil
...
% define coils -> fill factor is 0.4
s.defineCoil('PrimaryCoil', 'voltage', 'stranded');
s.addMeshZone2Coil('PrimaryCoil','PrimaryCoilArm1',gv_Np,-1,0.4);
s.addMeshZone2Coil('PrimaryCoil','PrimaryCoilArm2',gv_Np,1,0.4);

s.defineCoil('SecondaryCoil', 'voltage', 'stranded');
s.addMeshZone2Coil('SecondaryCoil','SecondaryCoilArm1',gv_Ns,1,0.4);
s.addMeshZone2Coil('SecondaryCoil','SecondaryCoilArm2',gv_Ns,-1,0.4);

s.setCoilVoltage('PrimaryCoil',@(t) gv_Vp*sin(2*pi*gv_freq*t));
s.setCoilVoltage('SecondaryCoil', 0);
