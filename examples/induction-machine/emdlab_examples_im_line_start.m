...
m.aux_addCircularAirGapInterface('ag',0,0,gv_ISD/2-gv_g,0,0,gv_ISD/2,1);
...
for i = 1:400
    ...
    te_mst(end+1) = s.evalTorqueByMST3(0,0,gv_ISD/2-gv_g/2,gv_g,1000);

    % solve mechanical equation
    wm(end+1) = wm(end) + timeStep * (te(end) - 5.1*sign(wm(end)) - 0.002*wm(end)) / 0.0025;

    % rotate moving region
    s.rotateMovingRegion('moving_1', wm(end)*timeStep);
