function MeshZone = emdlab_m2d_mg10(f, v, disFunc, maxIteration)
...
while count <= maxIteration
    ...
    d=disFunc(p); ix=d>geps;
    % Numerical gradient
    dgradx=(disFunc([p(ix,1)+deps,p(ix,2)])-d(ix))/deps;
    dgrady=(disFunc([p(ix,1),p(ix,2)+deps])-d(ix))/deps;
...
t = t.ConnectivityList(disFunc(t.incenter)<-geps,:);
