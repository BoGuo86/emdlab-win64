...
g = g.newcbwd('rap1',p.hg,'L2',-1,'A9',1,'L9',1,'A8',1,'A2',-1);
% g = g.newdDM('rap1',p.hg,'rap1');
...
g = g.newcbwd('sap1',p.hg,'L7',-1,'A7',-1,'A10',1,'L10',1,'A11',-1);
% g = g.newdDM('sap1',p.hg,'sap1');
%% creation of mesh
m = MDBCT(g);clear g;
m = m.setmzColor('r1','r');
m = m.setmzColor('s1','r');
...
for i = 1:2:6
    m = m.crotatemz(['r',num2str(i+2)],['r',num2str(i)],pi/4);
    m = m.crotatemz(['r',num2str(i+3)],['r',num2str(i+1)],pi/4);
end
for i = 1:2:10
...
for i = 1:3
    m = m.crotatemz(['rap',num2str(i+1)],['rap',num2str(i)],pi/4);
end
for i = 1:5
    m = m.crotatemz(['sap',num2str(i+1)],['sap',num2str(i)],pi/6);
end
%% joining mesh
temp = getlist('r',1:8);
...
m = m.rotatemz('rap3',-pi/8);
m = m.rotatemz('rap4',-pi/8);
m = m.ggmesh;
Nts = sum(m.tzi(:,13));
Ntr = sum(m.tzi(:,12));
%% solver setting
...
%     s.plotBmag('stator','rotor')
    Bxs(:,i) = s.B(s.m.tzi(:,13),1);
    Bys(:,i) = s.B(s.m.tzi(:,13),2);
    
    Bxr(:,i) = s.B(s.m.tzi(:,12),1);
    Byr(:,i) = s.B(s.m.tzi(:,12),2);
    
%     F(i) = getframe(gcf);
    
temp = temp + xrotorAngle(2);
    if temp <= pi/2
        s.m = s.m.rotatemz('rotor',xrotorAngle(2));
        s.m = s.m.rotatemz('rap1',xrotorAngle(2));
        s.m = s.m.rotatemz('rap2',xrotorAngle(2));
        s.m = s.m.rotatemz('rap3',xrotorAngle(2));
        s.m = s.m.rotatemz('rap4',xrotorAngle(2));
    else
%         temp = 0;
%         s.m = s.m.rotatemz('rotor',-pi/2);
%         s.m = s.m.rotatemz('rap1',-pi/2);
%         s.m = s.m.rotatemz('rap2',-pi/2);
    end
