...
dBxdt = abs(Bxr(:,2:end)-Bxr(:,1:end-1))/dt;
dBydt = abs(Byr(:,2:end)-Byr(:,1:end-1))/dt;
...
TotalHyst = diag(s.m.gta(s.m.tzi(:,12))) * physt*7650;

TotalHyst = k1 * sum(TotalHyst) * 55e-9 * 4 * dt/period

close all
t = s.m.t(s.m.tzi(:,12),1:3);
tt = 1:3*size(t,1);
t = t';
t = t(:);
tt = reshape(tt,3,[]);
...
colormap jet;
set(gcf,'Color','k')
title('Hysteresis Loss Density [W/m^3]',...
    'color','w','fontsize',15);
