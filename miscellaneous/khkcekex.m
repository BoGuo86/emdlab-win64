...
[k,res] = fmincon(@evalObjective,x0,[],[],[],[],[0 0 0 0]);

    function y = evalObjective(x)
        f = 50;
        y = sum(abs(p50'-x(1)*f*b50.^x(4)-x(2)*f^2*b50.^2-x(3)*f^1.5*b50.^1.5));
        f = 100;
        y = y + sum(abs(p100'-x(1)*f*b100.^x(4)-x(2)*f^2*b100.^2-x(3)*f^1.5*b100.^1.5));
        f = 200;
        y = y + sum(abs(p200'-x(1)*f*b200.^x(4)-x(2)*f^2*b200.^2-x(3)*f^1.5*b200.^1.5));
        f = 400;
        y = y + sum(abs(p400'-x(1)*f*b400.^x(4)-x(2)*f^2*b400.^2-x(3)*f^1.5*b400.^1.5));
        f = 1000;
        y = y + sum(abs(p1000'-x(1)*f*b1000.^x(4)-x(2)*f^2*b1000.^2-x(3)*f^1.5*b1000.^1.5));
        f = 2500;
        y = y + sum(abs(p2500'-x(1)*f*b2500.^x(4)-x(2)*f^2*b2500.^2-x(3)*f^1.5*b2500.^1.5));
    end
...
subplot(321);hold on
f = 50;
plot(b,k(1)*f*b.^k(4)+k(2)*f^2*b.^2+k(3)*f^1.5*b.^1.5)
subplot(322);hold on
f = 100;
plot(b,k(1)*f*b.^k(4)+k(2)*f^2*b.^2+k(3)*f^1.5*b.^1.5)
subplot(323);hold on
