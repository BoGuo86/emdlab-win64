...
Na = length(alpha);
Nb = length(beta);
alpha = repmat(alpha,Nb,1);
beta = repmat(beta,Na,1);
alpha = alpha(:);
beta = beta(:);
R = zeros(Na*Nb,1);
for i = 1:Na*Nb
    k1 = xdata(:,2).^alpha(i) .* xdata(:,1).^beta(i);
    k2 = xdata(:,2).^2 .* xdata(:,1).^2;
    k = [k1,k2]\ydata;
    R(i) = sqrt(sum((ydata-k(1)*k1-k(2)*k2).^2));
end
...
subplot(321);hold on
f = 50;
plot(b,k(1)*f^alpha *b.^beta+k(2)*f^2*b.^2)
subplot(322);hold on
f = 100;
plot(b,k(1)*f^alpha *b.^beta+k(2)*f^2*b.^2)
subplot(323);hold on
