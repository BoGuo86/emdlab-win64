...
xdata = [b50',50*ones(length(b50),1)
    b100',100*ones(length(b100),1)
    b200',200*ones(length(b200),1)
    b400',400*ones(length(b400),1)
    b1000',1000*ones(length(b1000),1)
    b2500',2500*ones(length(b2500),1)];
ydata = [p50;p100;p200;p400;p1000;p2500];

k = lsqcurvefit(@evalObjective,x0,xdata,ydata,[0,0],[1,1]);

    function y = evalObjective(x,xdata)
        y = x(1)*xdata(:,1).*xdata(:,2)+x(2)*xdata(:,1).^2 .* xdata(:,2).^2; 
    end
