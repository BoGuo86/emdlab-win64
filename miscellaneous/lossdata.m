...
k = fmincon(@evalObjective,x0,[],[],[],[],[0,0],[1,1]);

    function y = evalObjective(x)
        f = 50;
        y = sum(abs(p50-x(1)*f*b50'-x(2)*f^2*b50'.^2));
        f = 100;
        y = y + sum(abs(p100-x(1)*f*b100'-x(2)*f^2*b100'.^2));
        f = 200;
        y = y + sum(abs(p200-x(1)*f*b200'-x(2)*f^2*b200'.^2));
        f = 400;
        y = y + sum(abs(p400-x(1)*f*b400'-x(2)*f^2*b400'.^2));
        f = 1000;
        y = y + sum(abs(p1000-x(1)*f*b1000'-x(2)*f^2*b1000'.^2));
        f = 2500;
        y = y + sum(abs(p2500-x(1)*f*b2500'-x(2)*f^2*b2500'.^2));
    end
