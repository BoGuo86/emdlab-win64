...
g = g.newdswdkps([-Wslot/2+Wmid+Rwire,0],[-Wslot/2+Wmid+Rwire,Wmid],'Nnodes',2);
g = g.newdswdkps([-Wslot/2+Wmid,Wmid+Rwire],[-Wslot/2,Wmid+Rwire],'Nnodes',2);
...
% dc loss for 1 ampere current
DCLoss = nr*nd*Lst*1e3/58e6/m.mzs.c11.area;
%% Calling Solver
...
for i = 1:nd
    for j = 1:nr
        s = s.setExcitation(['c',num2str(i),num2str(j)],sqrt(2),0,'C');
    end
end
