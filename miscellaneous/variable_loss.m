function variable_loss()

x0 = [1e-1,1e-1,1e-1,1e-1];
...
[k,res] = fmincon(@evalObjective,x0,...
    [],[],[],[],[0 0 0 0],[1,1,1,1]);

    function y = evalObjective(x)
        f = 50;
        y = sum(abs(p50-(x(1)+x(2)*b50')*f.*b50'-...
            (x(3)+x(4)*b50')*f^2 .* b50'.^2));
        f = 100;
        y = y + sum(abs(p100-(x(1)+x(2)*b100')*f .*b100'-...
            (x(3)+x(4)*b100')*f^2 .* b100'.^2));
        f = 200;
        y = y + sum(abs(p200-(x(1)+x(2)*b200')*f .*b200'-...
            (x(3)+x(4)*b200')*f^2 .* b200'.^2));
        f = 400;
        y = y + sum(abs(p400-(x(1)+x(2)*b400')*f .*b400'-...
            (x(3)+x(4)*b400')*f^2 .* b400'.^2));
        f = 1000;
        y = y + sum(abs(p1000-(x(1)+x(2)*b1000')*f .*b1000'-...
            (x(3)+x(4)*b1000')*f^2 .* b1000'.^2));
        f = 2500;
        y = y + sum(abs(p2500-(x(1)+x(2)*b2500')*f .*b2500'-...
            (x(3)+x(4)*b2500')*f^2 .* b2500'.^2));
    end
...
subplot(321);hold on
f = 50;
plot(b,(k(1)+k(2)*b)*f .*b+(k(3)+k(4)*b)*f^2 .* b.^2);
subplot(322);hold on
f = 100;
plot(b,(k(1)+k(2)*b)*f .*b+(k(3)+k(4)*b)*f^2 .* b.^2);
subplot(323);hold on
