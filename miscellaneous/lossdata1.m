...
k = fminsearch(@evalObjective,[1e-3,1e-4,1,1e-4,1e-5,1]);

    function y = evalObjective(x)
        f = 50;
        y = sum(p50-(x(1)+x(2)*b50'+x(3)*f)*f.*b50'-...
            (x(4)+x(5)*b50'+x(6)*f)*f^2 .* b50'.^2).^2;
        f = 100;
        y = y + sum(p100-(x(1)+x(2)*b100'+x(3)*f)*f .*b100'-...
            (x(4)+x(5)*b100'+x(6)*f)*f^2 .* b100'.^2).^2;
        f = 200;
        y = y + sum(p200-(x(1)+x(2)*b200'+x(3)*f)*f .*b200'-...
            (x(4)+x(5)*b200'+x(6)*f)*f^2 .* b200'.^2).^2;
        f = 400;
        y = y + sum(p400-(x(1)+x(2)*b400'+x(3)*f)*f .*b400'-...
            (x(4)+x(5)*b400'+x(6)*f)*f^2 .* b400'.^2).^2;
        f = 1000;
        y = y + sum(p1000-(x(1)+x(2)*b1000'+x(3)*f)*f .*b1000'-...
            (x(4)+x(5)*b1000'+x(6)*f)*f^2 .* b1000'.^2).^2;
        f = 2500;
        y = y + sum(p2500-(x(1)+x(2)*b2500'+x(3)*f)*f .*b2500'-...
            (x(4)+x(5)*b2500'+x(6)*f)*f^2 .* b2500'.^2).^2;
    end
...
subplot(321);hold on
f = 50;
plot(b,(k(1)+k(2)*b+k(3)*f)*f .*b+(k(4)+k(5)*b+k(4)*f)*f^2 .* b.^2);
subplot(322);hold on
f = 100;
plot(b,(k(1)+k(2)*b+k(3)*f)*f .*b+(k(4)+k(5)*b+k(4)*f)*f^2 .* b.^2);
subplot(323);hold on
