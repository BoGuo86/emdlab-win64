...
rkp(2,:) = R1*[cos(-pi/4),sin(-pi/4)];
rkp(11,:) = R1*[cos(pi/4),sin(pi/4)];
rkp(12,:) = rsh*[cos(pi/4),sin(pi/4)];
rkp(7,:) = R1*[cos(betar/2),sin(betar/2)];
rkp(8,:) = R0*[cos(gammar),sin(gammar)];
rkp(6,:) = [rkp(7,1),-rkp(7,2)];
rkp(5,:) = [rkp(8,1),-rkp(8,2)];
rkp(3,:) = protate(rkp(7,:),-pi/4);
rkp(10,:) = protate(rkp(6,:),pi/4);
rkp(4,:) = protate(rkp(8,:),-pi/4);
rkp(9,:) = protate(rkp(5,:),pi/4);
...
s.rs2 = getarcpoints([0,0],rkp(2,:),rkp(3,:),0.9*min(hr,hg),1);
s.rs3 = getlinepoints(rkp(3,:),rkp(4,:),0.9*min(hr,hg));
s.rs4 = getarcpoints([0,0],rkp(4,:),rkp(5,:),0.9*min(hr,hg),5);
s.rs5 = getlinepoints(rkp(5,:),rkp(6,:),0.9*min(hr,hg));
s.rs6 = getarcpoints([0,0],rkp(6,:),rkp(7,:),0.9*min(hr,hg),1);
s.rs7 = getlinepoints(rkp(7,:),rkp(8,:),0.9*min(hr,hg));
s.rs8 = getarcpoints([0,0],rkp(8,:),rkp(9,:),0.9*min(hr,hg),5);
s.rs9 = getlinepoints(rkp(9,:),rkp(10,:),0.9*min(hr,hg));
s.rs10 = getarcpoints([0,0],rkp(10,:),rkp(11,:),0.9*min(hr,hg),1);
...
[rp,rt] = distmesh2d1(@dpoly,fh,hr,[min(rpoly);max(rpoly)],rpoly,rpoly);

gkp1 = (R1+g/2)*[cos(pi/4),sin(pi/4)];
gkp2 = (R1+g/2)*[cos(-pi/4),sin(-pi/4)];
s.gs = getarcpoints([0,0],gkp1,gkp2,hg,2);

g1poly = [gkp1;s.gs;gkp2;rkp(2,:);s.rs2;rkp(3,:);s.rs3;rkp(4,:);s.rs4;...
    rkp(5,:);s.rs5;rkp(6,:);s.rs6;rkp(7,:);s.rs7;rkp(8,:);s.rs8;...
    rkp(9,:);s.rs9;rkp(10,:);s.rs10;rkp(11,:);gkp1];

% plot(g1poly(:,1),g1poly(:,2))

[g1p,g1t] = distmesh2d1(@dpoly,fh,hg,[min(g1poly);max(g1poly)],g1poly,g1poly);
...
ss4 = getarcpoints([0,0],skp(4,:),skp(5,:),0.9*min(hs,hg),2);
...
ss9 = getarcpoints([0,0],skp(9,:),skp(10,:),0.9*min(hs,hg),2);
...
ss14 = getarcpoints([0,0],skp(14,:),skp(15,:),0.9*min(hs,hg),2);
...
[sp,st] = distmesh2d1(@dpoly,fh,hs,[min(spoly);max(spoly)],spoly,spoly);
...
ss23 = getarcpoints([0,0],skp(19,:),skp(4,:),0.9*min(hs,hg),2);
ss24 = getarcpoints([0,0],skp(5,:),skp(20,:),0.9*min(hs,hg),2);
ss25 = getarcpoints([0,0],skp(20,:),skp(9,:),0.9*min(hs,hg),2);
ss26 = getarcpoints([0,0],skp(10,:),skp(21,:),0.9*min(hs,hg),2);
ss27 = getarcpoints([0,0],skp(21,:),skp(14,:),0.9*min(hs,hg),2);
ss28 = getarcpoints([0,0],skp(15,:),skp(22,:),0.9*min(hs,hg),2);
...
[g2p,g2t] = distmesh2d1(@dpoly,fh,hg,[min(g2poly);max(g2poly)],g2poly,g2poly);
...
[c1a1p,c1a1t] = distmesh2d1(@dpoly,fh,hg,[min(c1a1poly);max(c1a1poly)],c1a1poly,c1a1poly);
[c1a2p,c1a2t] = distmesh2d1(@dpoly,fh,hg,[min(c1a2poly);max(c1a2poly)],c1a2poly,c1a2poly);
[c2a1p,c2a1t] = distmesh2d1(@dpoly,fh,hg,[min(c2a1poly);max(c2a1poly)],c2a1poly,c2a1poly);
[c2a2p,c2a2t] = distmesh2d1(@dpoly,fh,hg,[min(c2a2poly);max(c2a2poly)],c2a2poly,c2a2poly);
[c3a1p,c3a1t] = distmesh2d1(@dpoly,fh,hg,[min(c3a1poly);max(c3a1poly)],c3a1poly,c3a1poly);
[c3a2p,c3a2t] = distmesh2d1(@dpoly,fh,hg,[min(c3a2poly);max(c3a2poly)],c3a2poly,c3a2poly);

close all
triplot(triangulation(g1t,g1p),'r');axis off equal
hold on
triplot(triangulation(g2t,g2p),'b');
hold on
triplot(triangulation(c1a1t,c1a1p),'m');
hold on
triplot(triangulation(c1a2t,c1a2p),'m');
hold on
triplot(triangulation(c2a1t,c2a1p),'color',[0.5,0.3,0.8]);
hold on
triplot(triangulation(c2a2t,c2a2p),'color',[0.5,0.3,0.8]);
hold on
triplot(triangulation(c3a1t,c3a1p),'color',[0.1,0.7,0.8]);
hold on
triplot(triangulation(c3a2t,c3a2p),'color',[0.1,0.7,0.8]);
hold on
triplot(triangulation(st,sp),'k');
hold on
triplot(triangulation(rt,rp),'k');
