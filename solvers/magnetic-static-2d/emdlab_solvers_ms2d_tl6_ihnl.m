...
classdef emdlab_solvers_ms2d_tl6_ihnl < handle & emdlab_solvers_ms2d_tlcp
    ...
    methods
        ...
        function obj = emdlab_solvers_ms2d_tl6_ihnl(m)
            ...
            % check the mesh class
            if ~isa(m,'emdlab_m2d_tmdb')
                MException('', 'Wrong mesh databsase class. It must be <emdlab_m2d_tmdb>.');
            end

            % generate second order mesh elements
            if strcmpi(m.etype,'TL3')
                m.ggmesh;
                m.gd2elements;
            end
        ...
        function solve(obj)
            ...
            % Assembeling [K]
            [Iindex, Jindex] = emdlab_flib_getij(6,1);
