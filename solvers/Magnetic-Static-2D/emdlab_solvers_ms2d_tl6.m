...
classdef emdlab_solvers_ms2d_tl6 < handle
    ...
    methods        
        ...
        function y = evalTorqueByMST(obj, radius)

            [br, bt, ~, ~] = getBrBtOnCircleSmooth(obj, 0, 0, radius, 10000);
            y = sum(br.*bt * 2*pi/10000) * radius^2 * obj.units.k_length^2 * obj.getDepth / (4 * pi * 1e-7);
        ...
        function [bx, by] = getBxByOnPoints(obj, x, y)

            index = pointLocation(triangulation(obj.m.cl(:,1:3), obj.m.nodes), [x;y]');
            validIndices = ~isnan(index);
            index = index(validIndices);
            x = x(validIndices);
            y = y(validIndices);
            if sum(validIndices)
                [bx, by] = emdlab_m2d_tl6_interpB(obj.m.cl, obj.m.nodes, obj.results.A, obj.m.JIT, index, x, y);
                bx = bx * (obj.units.k_magneticVectorPotential / obj.units.k_length);
                by = by * (obj.units.k_magneticVectorPotential / obj.units.k_length);
            else
                error('No valid x and y point for interpolation');
            end

        end

        function [bx, by] = getBxByOnPointsSmooth(obj, x, y)
            ...
            y = y(validIndices);
            if sum(validIndices)
                [bx, by] = emdlab_m2d_tl6_interpBSmooth(obj.m.cl, obj.m.nodes, obj.results.BxnSmooth, obj.results.BynSmooth, obj.m.JIT, index, x, y);
            else
                error('No valid x and y point for interpolation');
            end
        ...
        function [br, bt, px, py] = getBrBtOnCircle(obj, xc, yc, r, N)
            ...
            px = r * cos(tmp) + xc;
            py = r * sin(tmp) + yc;
            [bx,by] = obj.getBxByOnPoints(px,py);
        ...
        function [br, bt, px, py] = getBrBtOnCircleSmooth(obj, xc, yc, r, N)
            ...
            px = r * cos(tmp) + xc;
            py = r * sin(tmp) + yc;
            [bx,by] = obj.getBxByOnPointsSmooth(px,py);
            Brt = ext_xy2rt([px; py]', [bx; by]');
            br = Brt(:, 1);
            bt = Brt(:, 2);
...
%             cb.Limits(2) = 1.7;
        ...
        function varargout = plotBmagF(obj, N, varargin)
            ...
            t = 1:size(c, 1);
            t = reshape(t, 2, [])';
            patch(ax, 'faces', t, 'vertices', c, 'edgecolor', 'k', 'linewidth',0.1);
...
%             cb.Limits(2) = 1.7;
        ...
        function plotBrBtOnCircle(obj, xc, yc, r, N)
            t = linspace(0,2*pi,N);
            x = xc + r * cos(t);
            y = yc + r * sin(t);
            ...
            br = sqrt(bx.^2+by.^2);
            figure('Name', 'Br and Bt on circle [tesla]')
            subplot(211)
            plot(t,br)
            %       set(gca,'Xlim',[pAngle(1),pAngle(end)])
            xlabel('Mechanical Angle [Degree]')
            ylabel('B_r [Tesla]')
            title(['Rdaial Flux Density, Mean = ', num2str(mean(abs(br)))])
            subplot(212)
            plot(t,br)
            %       set(gca,'Xlim',[pAngle(1),pAngle(end)])
            xlabel('Mechanical Angle [Degree]')
            ylabel('B_r [Tesla]')
            title('Tangentional Flux Density')
        end
